...
def get_avg_precision_at_iou(gt_boxes, pred_boxes, iou_thr=0.5):
    ...
    for ithr, model_score_thr in enumerate(sorted_model_scores[:-1]):
        ...
        for img_id in list(img_ids):
#             print(str(img_id)+str(1))
            gt_boxes_img = gt_boxes[str(img_id)]
#             print(str(img_id)+str(1))
...
def mAP(gt_boxes,pred_boxes,iou_thr = 0.7):
...
#        pred_boxes = json.load(infile)
    ...
    print(avg_precs)
	#mAPfile = open('mapgen.txt',"a")
    #print('map: {:.2f}'.format(100*np.mean(avg_precs)))
    return 100*np.mean(avg_precs)
    #mAPfile.write('map: {:.2f}'.format(100*np.mean(avg_precs)))
    #mAPfile.close()
